...
namespace WindowsFormsApp1
{
    ...
    public class CLEFIACipherTest
    {
        ...
        public static byte[] F0(byte[] K, byte[] text)
        {
        ...
        public static byte[] F1(byte[] K, byte[] text)
        {
            byte[] afterKeyRound = XOR(text, K);
            // Результат после применения S-блоков
        ...
        public static byte[][] GFN_(byte[][] roundKeys, byte[][] text, int rounds)
        {
            ...
            for (int i = 0; i < rounds; i++)
            {
                ...
                byte[] f0 = F0(roundKeys[roundKeyCount], input[0]);
                byte[] f1 = F1(roundKeys[roundKeyCount + 1], input[2]);
        ...
        {
            string hexPlainText = StringToHex(P);
            Console.WriteLine($"HEX Plain text {hexPlainText}");
            ...
            byte[] plainTextBytes = HexStringToByteArray(hexPlainText);
            // Ключевое забеливание 
            ...
            byte[][] result = GFN_(roundKeys, plainTextAfterWhitening, 18);
            // Ключевое забеливание
        ...
        public static byte[][] InverseGFN_(byte[][] roundKeys, byte[][] text, int rounds)
        {
            ...
            for (int i = 0; i < rounds; i++)
            {
                ...
                // Результат применения F-функций
                byte[] f0 = F0(roundKeys[roundKeyCount + 1], input[0]);
                byte[] f1 = F1(roundKeys[roundKeyCount], input[2]);
